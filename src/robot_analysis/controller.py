...
class ControllerABC(ABC):
    def __init__(self, t_step: float, u_name: str = ''):
        self._t_step = t_step
        self._u_name = u_name
        self._u: NDArray
    ...
    @abstractmethod
    def make_step(self, y: NDArray, setpoint: NDArray):
        pass
...
class FeedforwardPID(ControllerABC):
    def __init__(
        ...
        super().__init__(t_step, u_name)
        self._feedforward = feedforward_term
        self._kP = kP
        self._kI = kI
        self._kD = kD
        self._eI = np.zeros((n, 1)) * 1.0
        self._eD = np.zeros((n, 1)) * 1.0
        self._eI_max = eI_max
        self._u_max = u_max
